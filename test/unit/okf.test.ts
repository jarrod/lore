...
describe("OKF primitives", () => {
  ...
  test("derives trust and freshness", () => {
    ...
    expect(trustTier({ type: "X", verified: { by: "process:ci", at: "2026-01-01" } })).toBe(
      "machine_confirmed",
    );
    expect(trustTier({ type: "X", verified: [{ by: "human:j", at: "2026-01-01" }] })).toBe(
      "human_reviewed",
    );
  ...
  test("extracts Markdown and typed edges", () => {
    expect(
      extractMarkdownEdges(
        "[Target](../systems/okta.md#auth) ![Image](x.md) [Web](https://x.test)",
        "capabilities/id",
      ),
    ).toEqual([{ rel: "links_to", dst: "systems/okta", origin: "markdown" }]);
    expect(
      extractTypedEdges({ type: "X", "x-okf": { rel: [["depends_on", "systems/okta"]] } }),
  ...
  test("extracts a section with descendants", () => {
    const body = "# Root\n\n## Authentication\n\nA\n\n### Sessions\n\nB\n\n## Other\n\nC\n";
    expect(extractSection(body, "authentication")).toBe(
      "## Authentication\n\nA\n\n### Sessions\n\nB\n\n",
    );
  });
...
`);
  ...
  test("creates safe visualisation filenames and browser commands", () => {
    ...
    expect(browserCommand("C:\\graph.html", "win32")).toEqual([
      "cmd",
      "/c",
      "start",
      "",
