...
beforeAll(() => {
  ...
  if (built.exitCode !== 0)
    throw new Error(`Could not build integration executable: ${built.stderr.toString()}`);
...
async function cli(
  ...
  return runProcess(binary, [...args, "--bundle", targetBundle], stdin);
}

async function sourceCli(
  args: string[],
): Promise<{ exitCode: number; stdout: string; stderr: string }> {
  return runProcess("bun", [path.join(project, "src/cli.ts"), ...args]);
}

async function compiledCli(
  args: string[],
): Promise<{ exitCode: number; stdout: string; stderr: string }> {
...
describe("CLI protocol", () => {
  test("returns global and command help as successful JSON", async () => {
    ...
    expect(globalData.commands.map((command: { name: string }) => command.name)).toEqual([
      "init",
      "info",
      "index",
      "find",
    ...
    expect(commandData.options).toEqual(
      expect.arrayContaining([expect.objectContaining({ flag: "--to" })]),
    );
    ...
    expect(visualiseData.options).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ flag: "--open" }),
        expect.objectContaining({ flag: "--max-nodes" }),
      ]),
    );

    const reset = await cli(["reset", "--help"]);
    const resetData = JSON.parse(reset.stdout).data;
    expect(resetData.options).toEqual(
      expect.arrayContaining([
  ...
  test("returns compact JSON and stable validation exits", async () => {
    ...
    const warningCodes = JSON.parse(normal.stdout).data.warnings.map(
  ...
  test("consumes a complete OKF v0.2 bundle with provenance graph edges", async () => {
    ...
    const graph = await cli(
      ["graph", "computations/revenue", "--direction", "out", "--depth", "1"],
      undefined,
      okfBundle,
    );
    const graphData = JSON.parse(graph.stdout).data as {
      edges: Array<{ rel: string; origin: string }>;
    };
    expect(
      graphData.edges
        .filter((edge) => edge.origin === "okf")
        .map((edge) => edge.rel)
        .sort(),
    ).toEqual(["attester", "executor", "source"]);

    const metric = await cli(["get", "metrics/revenue"], undefined, okfBundle);
    const metricData = JSON.parse(metric.stdout).data as {
  ...
  test("classifies invalid concepts, unsafe links, structurally invalid reserved files and case collisions", async () => {
    ...
    writeFileSync(
      path.join(invalidBundle, "unsafe.md"),
      "---\ntype: Reference\n---\n# Unsafe\n\n[Escape](../../outside.md)\n",
    );
    ...
    const codes = JSON.parse(checked.stdout).data.errors.map(
      (finding: { code: string }) => finding.code,
    );
  ...
  test("enforces reserved-file conformance and warns on malformed optional OKF families", async () => {
    ...
    writeFileSync(
      path.join(validationBundle, "index.md"),
      '---\nokf_version: "0.2"\nextra: invalid\n---\n# Knowledge\n',
    );
    writeFileSync(
      path.join(validationBundle, "log.md"),
      "# Log\n\n## 2026-01-01\n- Older\n\n## 2026-02-01\n- Newer\n",
    );
    ...
    writeFileSync(
      path.join(validationBundle, "optional.md"),
      "---\ntype: Attested Computation\nstatus: reviewed\nstale_after: tomorrow\ntags: invalid\ngenerated:\n  at: yesterday\nverified: false\nsources:\n  - id: policy\n  - id: policy\n---\nClaim.[^missing]\n",
    );

    const checked = await cli(["check"], undefined, validationBundle);
    expect(checked.exitCode).toBe(3);
    const data = JSON.parse(checked.stdout).data as {
      errors: Array<{ code: string; reason?: string }>;
      warnings: Array<{ code: string }>;
    };
    expect(
      data.errors.filter((finding) => finding.code === "INVALID_RESERVED_FILE").length,
    ).toBeGreaterThanOrEqual(3);
    expect(data.warnings.map((finding) => finding.code)).toEqual(
      expect.arrayContaining([
  ...
  test("reports malformed typed metadata as invalid OKF during queries", async () => {
    ...
    writeFileSync(
      path.join(typedBundle, "typed-invalid.md"),
  ...
  if (process.platform !== "win32")
    test("refuses concept paths that resolve outside the bundle", async () => {
      ...
      const merge = await cli(
        ["put", "systems/escaped"],
        JSON.stringify({ mode: "merge", frontmatter: { title: "Changed" } }),
      );
      ...
      const create = await cli(
        ["put", "linked-directory/new"],
        JSON.stringify({ mode: "create", frontmatter: { type: "System" }, body: "# New\n" }),
      );
  ...
  test("creates, merges and guards concepts without metadata loss", async () => {
    ...
    const conflict = await cli(
      ["put", "systems/new-system"],
      JSON.stringify({ mode: "merge", expected_hash: "wrong", frontmatter: { title: "Lost" } }),
    );
    expect(conflict.exitCode).toBe(5);
    expect(JSON.parse(conflict.stderr).error.code).toBe("MUTATION_CONFLICT");

    const deniedReplace = await cli(
      ["put", "systems/new-system"],
      JSON.stringify({ mode: "replace", frontmatter: { type: "System" }, body: "# Replacement\n" }),
    );
    ...
    const replaced = await cli(
      ...
      JSON.stringify({
        ...
        frontmatter: { type: "System", title: "Replacement" },
        body: "# Replacement\n",
      }),
    );
    ...
    const fileCreated = await cli(
      ["put", "systems/from-file"],
      JSON.stringify({ mode: "create", frontmatter: { type: "System" }, body_file: bodyFile }),
    );
    expect(fileCreated.exitCode).toBe(0);
    expect(readFileSync(path.join(bundle, "systems/from-file.md"), "utf8")).toContain(
      "# From file",
    );

    const unsafe = await cli(
      ["put", "systems/unsafe-content"],
      JSON.stringify({
        mode: "create",
        frontmatter: { type: "System", resource: "../../outside.md" },
        body: "# Unsafe\n",
      }),
    );
  ...
  test("reports a committed mutation when derived index refresh needs recovery", async () => {
    ...
    const result = await cli(
      ["put", "saved"],
      JSON.stringify({ mode: "create", frontmatter: { type: "Concept" }, body: "# Saved\n" }),
      mutationBundle,
    );
    expect(result.exitCode).toBe(0);
    expect(JSON.parse(result.stdout).data.index).toEqual({
      current: false,
      recovery: "lore index --rebuild",
    });
  ...
  test("previews and performs a recoverable knowledge reset", async () => {
    ...
    const stale = await cli(
      ["reset", "--knowledge", "--confirm", previewData.confirmation_token],
      undefined,
      resetBundle,
    );
    ...
    const resetData = JSON.parse(reset.stdout).data as {
      backup: string;
      concepts: number;
      removed: { concepts: number; files: number };
      cache: { current: boolean };
    };
    ...
    expect(path.dirname(resetData.backup)).toBe(
      realpathSync(path.join(resetRepository, ".lore", "backups")),
    );
    expect(existsSync(path.join(resetData.backup, "systems/okta.md"))).toBeTrue();

    const info = await cli(["info"], undefined, resetBundle);
    expect(JSON.parse(info.stdout).data).toEqual(
      expect.objectContaining({ concepts: 0, edges: 0 }),
    );
  });
  ...
  test("permanently resets knowledge only with a mode-specific token", async () => {
    ...
    const recoverableToken = JSON.parse(recoverablePreview.stdout).data
      .confirmation_token as string;
    const permanentPreview = await cli(
      ["reset", "--knowledge", "--no-backup"],
      undefined,
      resetBundle,
    );
    const permanentData = JSON.parse(permanentPreview.stdout).data as {
      confirmation_token: string;
      mode: string;
      recoverable: boolean;
    };
    expect(permanentData).toEqual(
      expect.objectContaining({ mode: "permanent", recoverable: false }),
    );
    expect(permanentData.confirmation_token).not.toBe(recoverableToken);

    const wrongMode = await cli(
      ["reset", "--knowledge", "--no-backup", "--confirm", recoverableToken],
      undefined,
      resetBundle,
    );
    ...
    const reset = await cli(
      ["reset", "--knowledge", "--no-backup", "--confirm", permanentData.confirmation_token],
      undefined,
      resetBundle,
    );
    expect(reset.exitCode).toBe(0);
    ...
    expect(readdirSync(path.join(resetRepository, ".lore")).sort()).toEqual(["cache", "knowledge"]);
  ...
  test("generates complete and rooted visualisations as disposable HTML", async () => {
    ...
    const completeData = JSON.parse(complete.stdout).data as {
      path: string;
      scope: string;
      root: null;
      nodes: number;
    ...
    expect(path.basename(path.dirname(completeData.path))).toBe(
      path.basename(path.dirname(completePath)),
    );
    expect(existsSync(completeData.path)).toBeTrue();
    expect(completeData).toEqual(
      expect.objectContaining({ scope: "bundle", root: null, nodes: 9, edges: 9, opened: false }),
    );
    ...
    expect(
      (await cli(["visualise", "--output", completePath], undefined, visualBundle)).exitCode,
    ).toBe(0);
    ...
    expect(JSON.parse(rooted.stdout).data).toEqual(
      expect.objectContaining({ scope: "rooted", root: "capabilities/payments" }),
    );
    expect(readFileSync(rootedPath, "utf8")).toContain("capabilities/payments");

    const tooLarge = await cli(
      ["visualise", "--max-nodes", "1", "--output", path.join(root, "too-large.html")],
      undefined,
      visualBundle,
    );
    ...
    const invalidWholeOptions = await cli(
      ["visualise", "--depth", "2", "--output", path.join(root, "invalid.html")],
      undefined,
      visualBundle,
    );
    expect(invalidWholeOptions.exitCode).toBe(2);
    const insideOutput = path.join(visualBundle, "generated", "graph.html");
    const insideBundle = await cli(
      ["visualise", "--output", insideOutput],
      undefined,
      visualBundle,
    );
