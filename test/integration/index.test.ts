...
describe("derived index", () => {
  test("indexes, searches, traverses and rebuilds equivalently", async () => {
    ...
    const results = findConcepts(opened.db, "customer identity", { limit: 10 }) as Array<{
      id: string;
      status: string | null;
      trust: string;
    }>;
    ...
    expect(
      (findConcepts(opened.db, "customer", { tag: "identity" }) as Array<{ id: string }>).map(
        (row) => row.id,
      ),
    ).toEqual(["capabilities/customer-identity"]);
    expect(
      (
        findConcepts(opened.db, "identity", { type: "Team", scope: "teams" }) as Array<{
          id: string;
        }>
      )[0]?.id,
    ).toBe("teams/identity");
    ...
    expect(graph.nodes).toEqual(
      expect.arrayContaining([expect.objectContaining({ id: "systems/okta" })]),
    );
    const complete = bundleGraph(opened.db);
    expect(complete.nodes).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: "references/orphan" }),
        expect.objectContaining({ id: "systems/missing", missing: true }),
      ]),
    );
    expect(bundleGraph(opened.db, "owned_by").nodes.map((node) => node.id)).toEqual([
    ...
    expect(
      (findConcepts(opened.db, "customer absentterm", { limit: 10 }) as Array<{ id: string }>).some(
        (row) => row.id === "capabilities/customer-identity",
      ),
    ).toBeTrue();
    ...
    expect((findConcepts(opened.db, "minimal", {}) as Array<{ id: string }>)[0]?.id).toBe(
      "minimal-only",
    );
    ...
    writeFileSync(
      path.join(pathDirectory, "a.md"),
      "---\ntype: Concept\nx-okf:\n  rel:\n    - [next, paths/b]\n---\n",
    );
    writeFileSync(
      path.join(pathDirectory, "b.md"),
      "---\ntype: Concept\nx-okf:\n  rel:\n    - [next, paths/c]\n---\n",
    );
    writeFileSync(path.join(pathDirectory, "c.md"), "---\ntype: Concept\n---\n");
    await refreshIndex(opened.db, bundle);
    expect(
      shortestPath(opened.db, "paths/a", "paths/c", { direction: "out", maxDepth: 1 }).found,
    ).toBeFalse();
    expect(
      shortestPath(opened.db, "paths/a", "paths/c", { direction: "out", maxDepth: 2 }).found,
    ).toBeTrue();
    expect(
      shortestPath(opened.db, "paths/a", "paths/c", { direction: "in", maxDepth: 8 }).found,
    ).toBeFalse();
...
`,
    ...
    for (const token of [
      ...
      expect(
        (findConcepts(opened.db, token, {}) as Array<{ id: string }>).map((row) => row.id),
      ).toEqual(["semantic-search"]);
    }
    for (const token of [
      "destinationnoisetoken",
      "imagenoisetoken",
      "languageonlytoken",
      "htmlattributetoken",
      "htmlblocktoken",
    ...
    await refreshIndex(opened.db, bundle);
    const idsAfter = (
      findConcepts(opened.db, "customer identity", { limit: 10 }) as Array<{ id: string }>
  ...
  test("rebuilds an existing cache without a compatible schema", async () => {
    ...
    expect(opened.db.query("SELECT value FROM meta WHERE key='schema_version'").get()).toEqual({
      value: "4",
    });
