...
import { EXIT } from "../protocol/errors";
...
export async function runCheck(
  ...
  for (const concept of concepts) {
    ...
    if (prior && prior !== concept.id)
      errors.push({ code: "CASE_COLLIDING_CONCEPT_IDS", concept: concept.id, other: prior });
    else lowerIds.set(lower, concept.id);
    if (isStale(concept.frontmatter))
      warnings.push({
        code: "STALE_CONCEPT",
        concept: concept.id,
        stale_after: concept.frontmatter.stale_after,
      });
    for (const target of unsafeMarkdownTargets(concept.body, concept.id))
      errors.push({ code: "RELATIONSHIP_ESCAPES_BUNDLE", concept: concept.id, target });
    for (const target of unsafeOkfTargets(concept.frontmatter, concept.id))
      errors.push({ code: "OKF_PATH_ESCAPES_BUNDLE", concept: concept.id, target });
  ...
  for (const concept of concepts) {
    for (const edge of concept.edges) {
      ...
      if (ids.has(edge.dst)) {
        if (status.get(edge.dst) === "deprecated")
          warnings.push({
            code: "DEPRECATED_CONCEPT_REFERENCED",
            concept: concept.id,
            target: edge.dst,
          });
      } else {
...
function classifyFailure(error: unknown): string {
  ...
  if (message.includes("x-okf.rel") || message.includes("relationship"))
    return "MALFORMED_TYPED_RELATIONSHIP";
...
function sortFindings(findings: Finding[]): void {
  findings.sort((a, b) =>
    compareStrings(
      `${a.code}\0${a.concept ?? a.path ?? ""}`,
      `${b.code}\0${b.concept ?? b.path ?? ""}`,
    ),
  );
}
    ...
    if (parsed.frontmatter) {
      if (!rootIndex) {
        errors.push({
          ...
          path: relative,
          reason: "Only the bundle-root index.md may contain frontmatter",
        });
      } else {
        ...
        if (keys.some((key) => key !== "okf_version")) {
          errors.push({
            code: "INVALID_RESERVED_FILE",
            path: relative,
            reason: "Root index.md frontmatter may contain only okf_version",
          });
        }
        ...
        if (version !== undefined && (typeof version !== "string" || !version.trim())) {
          errors.push({
            code: "INVALID_RESERVED_FILE",
            path: relative,
            reason: "okf_version must be a non-empty string",
          });
        }
      }
    }
...
function parseReservedFile(
  ...
  if (!match) {
    errors.push({
      code: "INVALID_RESERVED_FILE",
      path: relative,
      reason: "Reserved file has unterminated YAML frontmatter",
    });
    return undefined;
  }
  try {
    ...
    if (!frontmatter || typeof frontmatter !== "object" || Array.isArray(frontmatter)) {
      errors.push({
        code: "INVALID_RESERVED_FILE",
        path: relative,
        reason: "Reserved file frontmatter must be a mapping",
      });
      return undefined;
    }
    return {
    ...
    errors.push({
      code: "INVALID_RESERVED_FILE",
      path: relative,
      reason: "Reserved file has invalid YAML frontmatter",
    });
    return undefined;
  }
}
...
function validateIndexBody(body: string, relative: string, errors: Finding[]): void {
  if (!markdownHeadings(body).length) {
    errors.push({
      code: "INVALID_RESERVED_FILE",
      path: relative,
      reason: "index.md must group entries under at least one heading",
    });
  }
}
...
function validateLogBody(body: string, relative: string, errors: Finding[]): void {
  ...
  if (!levelTwo.length || levelTwo.some((heading) => !isOkfDate(heading.title))) {
    errors.push({
      code: "INVALID_RESERVED_FILE",
      path: relative,
      reason: "log.md requires ISO YYYY-MM-DD level-two date headings",
    });
    return;
  }
  ...
  if (dates.some((date, index) => index > 0 && date > dates[index - 1]!)) {
    errors.push({
      code: "INVALID_RESERVED_FILE",
      path: relative,
      reason: "log.md date headings must be newest first",
    });
  }
}
...
function markdownHeadings(body: string): Array<{ level: number; title: string }> {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (atx) {
      headings.push({ level: atx[1]!.length, title: atx[2]!.trim() });
      continue;
    }
...
function validateStandardFrontmatter(concept: Concept, warnings: Finding[]): void {
  ...
  if (
    fm.stale_after !== undefined &&
    (typeof fm.stale_after !== "string" || !isOkfDate(fm.stale_after))
  ) {
    warnings.push({ code: "INVALID_STALE_AFTER", concept: concept.id, value: fm.stale_after });
  }
  if (
    fm.tags !== undefined &&
    (!Array.isArray(fm.tags) || fm.tags.some((tag) => typeof tag !== "string"))
  ) {
...
function validateGenerated(concept: Concept, warnings: Finding[]): void {
  ...
  if (
    !isMapping(generated) ||
    !nonEmptyString(generated.by) ||
    (generated.at !== undefined && !isIsoDateTime(generated.at))
  ) {
...
function validateVerified(concept: Concept, warnings: Finding[]): void {
  ...
  if (
    !entries.length ||
    entries.some(
      (entry) => !isMapping(entry) || !nonEmptyString(entry.by) || !isIsoDateTime(entry.at),
    )
  ) {
...
function validateSources(concept: Concept, warnings: Finding[]): void {
  ...
  if (
    concept.frontmatter.usage_window !== undefined &&
    !isUsageWindow(concept.frontmatter.usage_window)
  ) {
  ...
  if (new Set(ids).size !== ids.length)
    warnings.push({ code: "DUPLICATE_SOURCE_ID", concept: concept.id });
  const cited = new Set([...concept.body.matchAll(/\[\^([^\]]+)\]/g)].map((match) => match[1]!));
  for (const id of cited) {
    if (!ids.includes(id))
      warnings.push({ code: "SOURCE_FOOTNOTE_WITHOUT_SOURCE", concept: concept.id, source_id: id });
  }
}

function validateAttestedComputation(concept: Concept, warnings: Finding[]): void {
  const fm = concept.frontmatter;
  if (!nonEmptyString(fm.runtime))
    warnings.push({ code: "ATTESTED_COMPUTATION_MISSING_RUNTIME", concept: concept.id });
  if (
    fm.parameters !== undefined &&
    (!Array.isArray(fm.parameters) ||
      fm.parameters.some(
        (parameter) =>
          !isMapping(parameter) ||
  ...
  if (
    isMapping(fm.executor) &&
    fm.executor.receipt !== undefined &&
    (!Array.isArray(fm.executor.receipt) ||
      fm.executor.receipt.some((field) => !nonEmptyString(field)))
  ) {
  ...
  if (externalComputation && !nonEmptyString(fm.computation))
    warnings.push({ code: "INVALID_COMPUTATION_PATH", concept: concept.id });
...
function validSource(value: unknown): value is Record<string, unknown> {
  ...
  if (
    ...
    (typeof value.last_modified !== "string" || !isOkfDate(value.last_modified))
  )
    return false;
