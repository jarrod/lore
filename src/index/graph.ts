...
export function bundleGraph(
  ...
  const edges = (
    db
      .query(
        `SELECT src,rel,dst,origin FROM edge${rel ? " WHERE rel = ?" : ""} ORDER BY src,rel,dst,origin`,
      )
      .all(...params) as Array<{
      src: string;
      rel: string;
  ...
  const rows = db
    .query("SELECT id,type,title,status,trust FROM concept ORDER BY id")
    .all() as GraphNode[];
  const nodes = selectedIds ? rows.filter((node) => selectedIds.has(node.id)) : rows;
  const known = new Set(rows.map((node) => node.id));
  for (const edge of edges) {
    if (!known.has(edge.from) && !nodes.some((node) => node.id === edge.from))
      nodes.push({ id: edge.from, missing: true });
    if (!known.has(edge.to) && !nodes.some((node) => node.id === edge.to))
      nodes.push({ id: edge.to, missing: true });
  }
  ...
  for (let level = 0; level < maxDepth && frontier.length; level++) {
    ...
    for (const id of frontier.sort()) {
      for (const edge of adjacent(db, id, direction, rel)) {
        ...
        visited.add(neighbour);
        parent.set(neighbour, { previous: id, edge });
        next.push(neighbour);
        if (neighbour === to) {
...
function adjacent(db: Database, id: string, direction: Direction, rel?: string): GraphEdge[] {
  ...
  const rows = db
    .query(
      `SELECT src,rel,dst,origin FROM edge WHERE (${clauses.join(" OR ")})${relSql} ORDER BY src,rel,dst,origin`,
    )
    .all(...params) as Array<{ src: string; rel: string; dst: string; origin: string }>;
...
function nodeMetadata(db: Database, ids: string[]): GraphNode[] {
  ...
  return ids
    .map((id): GraphNode => (query.get(id) as GraphNode | null) ?? { id, missing: true })
    .sort((a, b) => compareStrings(a.id, b.id));
}

function compareEdges(a: GraphEdge, b: GraphEdge): number {
  return compareStrings(
    `${a.from}\0${a.rel}\0${a.to}\0${a.origin}`,
    `${b.from}\0${b.rel}\0${b.to}\0${b.origin}`,
  );
}
