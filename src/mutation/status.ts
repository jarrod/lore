...
export async function runStatus(bundle: string, args: string[]): Promise<unknown> {
  ...
  if (!rawStatus || !rawStatus.trim())
    throw invalidArgument("status requires a non-empty status value");
  const status = rawStatus.trim();
  if (!isOkfStatus(status))
    throw invalidArgument("status must be an OKF lifecycle value", {
      status,
      allowed: OKF_STATUSES,
    });
  const expectedHash = takeOption(args, "--expected-hash");
  ensureNoArgs(args);
  if (!existsSync(conceptPath(bundle, id)))
